...
- **Backlog = unchecked ISCs ∪ unchecked `## Remaining Work` (v2.19.0, ADVISORY — never a gate)** — "what does this ISA still owe" is the union of its open claims and its open Remaining Work items, and is computable from the artifact alone. `## Remaining Work` holds concrete work that was never an ISC of this run (follow-ups, handoffs); recording an item there is a complete disposition, requiring no card. It is **never required and never blocks** — an ISA with an empty or absent Remaining Work section passes, and its items are never counted toward Coverage (unlike ISCs). Report the backlog size for visibility; never fail on it. Unchecked ISCs stay in `## Claims`/`## Criteria` (ID-Stability forbids moving them into Remaining Work).
