...
1. **LLM-readable targets** — describe elements in natural language ("First story title link"), not CSS selectors (`.story-link:first-child`). general-purpose reviewer agents use accessibility snapshots to find elements by description.
...
Stories are executed by the **ReviewStories** workflow, which fans each story out to a parallel general-purpose reviewer agent:
...
- Keep story count per file under 8 (reviewer parallelism limit)
