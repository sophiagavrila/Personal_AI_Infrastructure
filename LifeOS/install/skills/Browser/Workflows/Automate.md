...
| `ai-agent` | Spawn `Task(subagent_type="general-purpose", prompt=resolved_template)` |
...
- **ai-agent:** Pass the full resolved template as the agent prompt
...
- **Unresolved params left in place.** ai-agent dispatches can handle `{param}` in their prompt and ask or infer the value. This is graceful degradation, not an error.
