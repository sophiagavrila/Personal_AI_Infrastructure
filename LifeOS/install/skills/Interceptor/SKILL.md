...
| **INPUT** | `click`, `type`, `keys`, `act <ref> [--trusted]`, `drag`, `scroll`, `select`, `focus` | Browser + native macOS input; `--trusted` for OS-level HID source state. `click --selector <css>` clicks by CSS selector (0.23.6+), returns the clickable `e<ref>`, and auto-escalates to an OS-level click when the synthetic one is rejected |
...
- **DOM-render screenshots don't paint UA default widget chrome — button/input contrast bugs pass invisibly.** A bare `<button>` with no CSS `background` renders the UA's white/#efefef background in the real browser (worst on iOS Safari), but the DOM-render capture paints it as if transparent, so a white-on-white contrast bug looks fine in the screenshot (2026-08-07: Surface share buttons shipped unreadable on mobile after a clean-looking capture). Contrast claims on form controls need `getComputedStyle(...).backgroundColor` (transparent = UA bg will show) or a `--pixel` capture, never the DOM-render default. Textarea/input *values* set via property are likewise not painted.
...
- **Overwriting `/opt/homebrew/bin/interceptor{,-daemon}` in place trips `OS_REASON_CODESIGNING`** — after `cp` over the old binaries, the `com.interceptor.daemon` LaunchAgent's spawn gets SIGKILLed by the kernel (`launchctl print` → `last exit reason = OS_REASON_CODESIGNING`) even though the build ad-hoc signed them. Re-sign the installed copies in place (`codesign --force --sign - /opt/homebrew/bin/interceptor-daemon` and `.../interceptor`), then `launchctl kickstart "gui/$(id -u)/com.interceptor.daemon"`. *(2026-07-28 upgrade; recurred 2026-08-10 — 92 SIGKILL crash reports in one 77-minute window. Update workflow step 4 is now the durable fix: stage → sign → atomic `mv`, never `cp` onto the live path.)*
