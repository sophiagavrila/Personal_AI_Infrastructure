...
**Version:** LifeOS 6.0.0 | Algorithm v6.18.0 | Memory v8.2
...
| `hooks/InstructionsLoadedHandler.hook.ts` | Validates instruction set at session start |
...
- **Version:** v6.18.0
- **Location:** `LIFEOS/ALGORITHM/` (canonical pointer: `LATEST` → `v6.18.0.md`)
- **CLI:** `bun LIFEOS/TOOLS/algorithm.ts`
- **Full doc:** `LIFEOS/DOCUMENTATION/Algorithm/AlgorithmSystem.md`
- **Doctrine highlights (v6.11.0):** twelve-section ISA in fixed order; closed enumeration of thinking capabilities; Capability-Name Audit Gate (phantom names = CRITICAL FAILURE); ID-stability rule (ISC IDs never re-number on edit); Cato cross-vendor audit MANDATORY at E4/E5 in VERIFY (Rule 2a). **v6.11.0 addition — Tier-Scaled Delegation Effort:** the Algorithm tier scales the effort level of spawned agents (the layer where effort is programmable — a hook cannot set main-loop effort per turn); E1 low → E5 ultracode-by-composition (Workflow orchestration + xhigh agents, reproducing `/effort ultracode`). Selected at the PLAN Delegation Gate; mirror table in `capabilities.md`. The v6.10.0 property-testing doctrine remains a separate unpromoted candidate. **v6.9.0 — Resume After Complete:** an edit landing on a `phase: complete` ISA with body diff triggers deterministic auto-rewind to `phase: learn`, increments `iteration`, writes `resumed_at` + `resumed_from_phase: complete` to frontmatter, appends a Decisions row, and appends to `MEMORY/OBSERVABILITY/isa-rework.jsonl`. **v6.8.0:** Frame-Drift Check at VERIFY (three-test rubric T1/T2/T3 + Cato adversarial second pass at E4/E5). **v6.7.0:** Context Sufficiency as universal LifeOS principle (Density Gate, Sufficiency Check, PLAN-entry Refresh, NATIVE-mode ambiguity flag).

### The Router

**The prompt-comprehension and execution-posture layer — every prompt passes through it before any work begins.**

Turns a raw prompt into a posture: mode (MINIMAL/NATIVE/ALGORITHM), effort tier (E1–E5), the stated goal, the model rung, and which agent/vendor runs the work. Four stages — classify (`hooks/EffortRouter.hook.ts`), route the effort (Algorithm tier→level policy, which produces a *level*), select the model (`LIFEOS/TOOLS/models.ts` four-level `EFFORT_MODEL` abstraction maps that level to a model), and dispatch the agent (dispatch-time `model` param). The Algorithm is the Router's biggest consumer, not part of it: the Router decides how to handle a prompt; the Algorithm/NATIVE executes within that decision. It exists to preserve dynamic range (fast on trivial work, deep on hard work) and to work around a harness limit — the main loop cannot switch its own model mid-turn, so the Router classifies up front and programs the model + effort of the agents the work dispatches.

- **Status:** Active (named + documented 2026-07-01)
- **Location:** `hooks/EffortRouter.hook.ts`, `LIFEOS/TOOLS/models.ts`, `LIFEOS/TOOLS/Inference.ts`, Algorithm Mode Classification doctrine
...
- **Version:** 8.2 (P1 proposal subtypes + session-rename landed 2026-05-25; autonomic loop + health gate + indicator preserved from 8.1)
...
Headless by default. Handles screenshots, multi-step sessions, authenticated browsing, scraping, and data extraction. Auth via persistent profiles (`--profile ~/.agent-browser/profiles/<site>`). Legacy built-in agents (BrowserAgent, UIReviewer, QATester) were removed 2026-06-10 — browser work delegates to general-purpose agents driving agent-browser. All web-based output must be verified through the **Interceptor skill** before showing to the user. Playwright is banned across PAI.
...
- **Decided:** 2026-06-10
- **Decision:** All Claude-family model routing speaks four effort LEVELS — `max` / `high` / `medium` / `low` — bound to concrete models in exactly one place: `EFFORT_MODEL` in `LIFEOS/TOOLS/models.ts` (today max→Opus, high→Opus, medium→Sonnet, low→Haiku — the designed max→Fable flip is held pending the fable/opus economics decision). Doctrine, hooks, skills, and agents state intent (the level); the mapping resolves the model. v6.18.0 adds a per-task `intelligence` level on `## Features` the dispatch reads (down-route-only, fail-expensive, E1–E5).
