    ...
    <header className="sticky top-0 z-50 bg-slate-950/80 backdrop-blur-md">
      ...
      <div className="border-b border-slate-800/50">
        <div className="max-w-[1920px] mx-auto px-4 sm:px-6">
          {/* Single flex row: logo · nav (grows + center-justifies its own
          ...
          <div className="flex items-center h-14 gap-4 lg:gap-6">
            ...
            <nav
              className="hidden md:flex flex-1 items-center justify-start xl:justify-center gap-1 min-w-0 overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
            >
              {lifeNav.map((item) => {
                ...
                return (
                  <Link
                    ...
                    className={cn(
                      ...
                      active ? "bg-white/15 text-white font-semibold" : "font-medium text-slate-400 hover:text-white hover:bg-white/5"
                    )}
                    style={fontStyle}
                  >
      ...
      <div className="border-b border-slate-800/30 bg-slate-950/60">
        <div className="max-w-[1920px] mx-auto px-4 sm:px-6">
          <div className="hidden md:flex items-center h-11 gap-3">
            <nav className="flex flex-1 items-center justify-start 2xl:justify-center gap-1 min-w-0 overflow-x-auto [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden">
              {systemNav.map((item) => {
                const active = isActive(item.href);
                const Icon = item.icon;
                return (
