        ...
        <div className="space-y-1 pt-1 border-t border-white/[0.04]">
          {entries.slice(0, 3).map((e) => (
            <div key={e.id} className="flex items-center gap-2">
              {statusDot(e.status)}
              <span className="text-[12px] text-zinc-500 font-mono">{e.id}</span>
              <span className="text-sm text-zinc-300 truncate">{e.title}</span>
            </div>
          ))}
          {entries.length > 3 && (
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 space-y-6">
      ...
      <div className="bg-white/[0.01] border border-white/[0.04] rounded-xl p-4">
        <div className="flex items-center justify-between text-[13px] text-zinc-400 mb-3 uppercase tracking-wide font-medium">
