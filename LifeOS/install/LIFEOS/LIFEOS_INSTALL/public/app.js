/**
 * LifeOS Installer v6.0 — Frontend Application
...
let steps = [
  ...
  { id: 'repository', name: 'LifeOS Repository', number: 5, status: 'pending' },
...
function renderDetection(data) {
  ...
  const items = [
    ...
    { icon: data.existing?.paiInstalled ? 'info' : 'check', label: 'Existing LifeOS', value: data.existing?.paiInstalled ? 'v' + (data.existing.paiVersion || '?') : 'Fresh install' },
...
function renderSummary(summary) {
  ...
  card.innerHTML = `
    ...
    <div class="summary-row"><span class="s-label">LifeOS Version</span><span class="s-value">v${summary.paiVersion}</span></div>
    ...
    <div class="summary-action">
      <p>To activate LifeOS, open a terminal and run:</p>
      <code>source ~/.zshrc && pai</code>
      <p class="summary-hint">This reloads your shell config and launches LifeOS for the first time.</p>
    </div>
  `;
