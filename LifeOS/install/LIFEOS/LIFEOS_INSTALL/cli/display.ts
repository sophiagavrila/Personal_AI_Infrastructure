/**
 * LifeOS Installer v6.0 — CLI Display Helpers
...
export function printStep(num: number, total: number, name: string): void {
  // Loud, unambiguous section frame — heavy double-line border in LifeOS blue,
...
export function printBanner(): void {
  ...
  print("");
  print(`                      ${c.navy}P${c.reset}${c.blue}A${c.reset}${c.lightBlue}I${c.reset} ${c.steel}|${c.reset} ${c.gray}LifeOS${c.reset}`);
  print("");
  ...
  print(`           ${c.navy}████${c.reset}        ${c.navy}████${c.reset}${c.lightBlue}████${c.reset}   ${sep}  ${c.navy}⬢${c.reset}  ${c.gray}LifeOS${c.reset}       ${c.silver}v6.0.0${c.reset}`);
  print(`           ${c.navy}████${c.reset}        ${c.navy}████${c.reset}${c.lightBlue}████${c.reset}   ${sep}  ${c.navy}⚙${c.reset}  ${c.gray}Algo${c.reset}      ${c.silver}v6.9.0${c.reset}`);
  print(`           ${c.navy}████████████████${c.reset}${c.lightBlue}████${c.reset}   ${sep}  ${c.lightBlue}✦${c.reset}  ${c.gray}Installer${c.reset} ${c.silver}v6.0${c.reset}`);
  print(`           ${c.navy}████████████████${c.reset}${c.lightBlue}████${c.reset}   ${sep}  ${bar}`);
...
export function printDetection(det: DetectionResult): void {
  ...
  if (det.existing.paiInstalled) {
    printInfo(`Existing LifeOS: v${det.existing.paiVersion || "unknown"} (fresh install will restore from backup if you allow it)`);
  } else {
    printInfo("Existing LifeOS: not detected (fresh install)");
  }
...
export function printSummary(summary: InstallSummary): void {
  ...
  print(`${c.navy}╠══════════════════════════════════════════════════╣${c.reset}`);
  print(`${c.navy}║${c.reset}  LifeOS Version:  ${c.white}v${summary.paiVersion}${c.reset}                             ${c.navy}║${c.reset}`);
