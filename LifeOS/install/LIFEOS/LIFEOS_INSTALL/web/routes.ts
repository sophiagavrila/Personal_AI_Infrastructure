/**
 * LifeOS Installer v6.0 — API Routes
...
async function startInstallation(): Promise<void> {
  ...
  try {
    ...
    // Unconditional template substitution — runs before validation so every
    // install path (skip-voice / skip-telegram included) lands substituted
    // identity values across CLAUDE.md, agents, hooks, skills, USER tree.
    const paiDirForSubst = installState.detection?.paiDir || `${process.env.HOME}/.claude`;
    await runTemplateSubstitution(installState, paiDirForSubst, emit);
    ...
    // reaches the operator over the websocket.
    const copyFailures = getCopyMissingFailures();
    if (copyFailures.length > 0) {
      broadcast({ type: "copy_failures", count: copyFailures.length, failures: copyFailures });
    }
