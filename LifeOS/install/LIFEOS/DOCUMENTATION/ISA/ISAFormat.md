...
**Required-when:** the four v6.5.0 fields populate together when `INTERVIEW_ELIGIBLE: true` (v6.7.0 extends eligibility to all ALGORITHM tiers). The two v6.7.0 fields populate when ANY Context Sufficiency check ran (Density Gate at any tier, Sufficiency Check at scaffold time, or plan-entry Refresh). The two v6.8.0 fields populate when Frame-Drift Check ran at verification — i.e., when ISA has non-null `principal_stated_goal`. If `principal_stated_goal: null` (or absent), `frame_drift: skipped_no_goal_literal`. Otherwise all six are omitted entirely (Bitter Pill discipline).
...
| **Deepest** (frontier multi-component work) | All seventeen* + Interview workflow run before building |

\* `## Dependencies` and `## Bridge Criteria` are **conditional-required**: mandatory when the ISA has any `parent:`/`children:`/cross-ISA relationship, omitted (like any empty section) for a standalone single-ISA task. `## Not yet specified` appears only when the work genuinely has fog. `## Language` appears only when a term has actually been confused — it is never required by substance, and an empty or speculative glossary is worse than none. `## Remaining Work` is **never required at any tier** — it appears only when the ISA genuinely still owes work that was never an ISC of the run, and its absence is never a gate failure. A hierarchical ISA that omits Dependencies/Bridge fails the gate; a standalone one that omits them passes.
...
### Verifier classes and execution tiers (v2.20.0)

Every claim closes through one of three **verifier classes** — the answer to *who or what can say no to this claim, and how*. The class is **derived from the existing `type` column**; there is no new field to author:
...
| **judged** | `eval` | a model bound to a stated rubric (Evals skill, `EvalRunner` suites, pass^k) | repeatable and evidence-producing; a failure flips the claim open and raises a finding, never blocks a ship |
...
- **Prose sections have no class.** Problem, Vision, Principles, Out of Scope, and fog steer scoring and scope; they can never fail. Only claims verify.
- **The schema is constant across domains; only the class mix varies.** An application skews deterministic, a story skews judged ("the wound surfaces before the midpoint" closes through a rubric), a watercolor skews attested ("the forewing gradient reads luminous" closes when the principal looks and says yes). Same sections, same claim shape, same Test Strategy contract.
- **An unavailable verifier means DEFER, never fail** (`LIFEOS/RULES/Verification.md`). The claim goes open-with-reason. An ISA never breaks; it shows honest state.

**Execution tier.** Optional eighth `## Test Strategy` column **`tier`**, after `severity`: `deep`, or blank = fast.

- **fast** (default): seconds. Runs in the dev loop and the deploy gate — the only tier that may block anything.
- **deep**: minutes to hours — soak, load, full browser sweeps, multi-sample eval suites. Runs on Bunker's cloud schedule; a failure alerts and flips the claim open on Pulse, never blocks work. `eval` rows are deep by default (multi-sample suites are slow by construction); mark one fast only when its suite provably runs in seconds.

Compatibility contract: `Bunker/src/isa.ts` (private implementation — the Bunker concept ships in DOCUMENTATION, the code is release-excluded) reads `cells[0..6]` only, so an eighth cell is ignored by all existing tooling (verified against source 2026-08-09). A row carrying `tier` writes the `severity` column too (blank when normal) so slots stay aligned — a bare `deep` landing in slot 6 is rejected loudly as an unknown severity, which is the intended failure. Scheduler execution of the deep tier is live in both planes (Bunker ISC-23/24): the local monitor runs deep rows on its schedule, and the cloud health worker runs `deep`-compiled checks on a top-of-hour cadence with the last verdict carried forward between runs. Deploy and close gates still run fast deterministic rows only, by doctrine.
...
When claims have real execution ordering, encode it on the claim line as a trailing parenthetical:
...
**Syntax.** `(after: ID[, ID...])` as the LAST parenthetical of the description, optionally before a final period. IDs are claim IDs from the same ISA (any legal form — `ISC-N`, `ISC-2.1`, `C4`, `H-AVAIL`). Backticked code spans and mid-line parentheticals never parse as edges, so a claim can *quote* the syntax safely. Edges are intra-ISA only; cross-ISA needs stay in `## Dependencies`.

**Semantics.** A claim is **takeable** when it is open (unchecked, not tombstoned), every `after:` blocker is resolved (checked or `[DROPPED…]`), and no fresh lock is held on it. The frontier is the set of takeable claims — "what can be worked right now," computed, never re-derived by rereading the whole ISA.

**The tool.** `bun LIFEOS/TOOLS/IsaFrontier.ts <cmd> <isa-path>`:

| Command | Does |
|---------|------|
| `frontier` | takeable claims + taken/blocked counts (`--json` for structured) |
...
**Concurrent sessions.** Locks are `wx`-created files at `MEMORY/STATE/isa-locks/<sha1(realpath)>/<claim-id>.lock` — atomic on one machine, which is where concurrent {{DA_NAME}} sessions live; the path is `realpathSync`-canonicalized so the MEMORY symlink's two legal spellings hash to one lock dir. **`--session` is the harness session UUID** — never a shared label like a name, which would make every session "the owner" and defeat the exclusion; re-claiming with your own UUID doubles as a heartbeat (rewrites the lock's timestamp), so long-running work re-claims periodically to stay fresh. Stale locks (>2h, `--ttl-hours` to tune) are surfaced and takeable. A session sharing an ISA claims before building and releases at close; a solo run never needs the protocol. Multi-session **planning** works the same way: when fog exceeds one window, the ISA holds the map and each scoping/research session takes a takeable unit (Algorithm v8.20.0 §Spend).
