...
**Version:** LifeOS 6.0.5 | Algorithm v6.24.0 | Memory v8.2
...
Turns a raw prompt into a posture: mode (MINIMAL/NATIVE/ALGORITHM), effort tier (E1–E5), the stated goal, the model rung, and which agent/vendor runs the work. Four stages — classify (`hooks/TheRouter.hook.ts`), route the effort (Algorithm tier→level policy, which produces a *level*), select the model (`LIFEOS/TOOLS/models.ts` four-level `EFFORT_MODEL` abstraction maps that level to a model), and dispatch the agent (dispatch-time `model` param). The Algorithm is the Router's biggest consumer, not part of it: the Router decides how to handle a prompt; the Algorithm/NATIVE executes within that decision. It exists to preserve dynamic range (fast on trivial work, deep on hard work) and to work around a harness limit — the main loop cannot switch its own model mid-turn, so the Router classifies up front and programs the model + effort of the agents the work dispatches.

- **Status:** Active (named + documented 2026-07-01)
- **Location:** `hooks/TheRouter.hook.ts`, `LIFEOS/TOOLS/models.ts`, `LIFEOS/TOOLS/Inference.ts`, Algorithm Mode Classification doctrine
